...
import { bookClassList, bookShelfList, deleteBookShelf } from '@/api/bookshelf'
...
export default {
  ...
  async created() {
        ...
        newRouter[2].children[1].children.push(
          {
            ...
            meta: {
              title: item.classesName + '(' + item.bookNum + ')'
            }
          }
        )
      })
    ...
    const result1 = await bookShelfList({
      pageNum: 1,
      pageSize: 10,
  ...
  methods: {
    ...
    del(_id, index) {
      this.$confirm('确定删除？', '提示', {
        ...
        const res = await deleteBookShelf({
          id: _id
        })
